...
class AttributesTest extends TestCase
{
	...
	$fakeAttribute = factory(Attributes::class)->create([
		'project_id'   =>$fakeProject->project_id,
		'purpose_name' =>$fakePurpose->system_name
	]);

        $results = Attributes::with('purpose')->get();
    ...
    {
        $fakeProject = factory(Project::class)->create();
        $fakeAttribute = factory(Attributes::class)->create(['project_id'=>$fakeProject->project_id]);

        $result= Attributes::with('project')->findOrFail($fakeAttribute->project_id);
